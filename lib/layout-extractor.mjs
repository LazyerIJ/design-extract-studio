...
function inPageWalk(opts) {
  ...
  // DOM-ancestry signature (tokens from <body> down) used to cross-match the
  // same box across breakpoints. Deliberately NO positional index: an
  // nth-among-siblings counter silently shifts when a same-token sibling is
  // conditionally removed at another width, producing wrong matches. Identical
  // repeated siblings therefore share a signature on purpose — the matcher
  // treats a non-unique signature as ambiguous and declines to match it.
...
export async function extractLayout({
  ...
  const { child, wsUrl, userDataDir } = await launchChrome(chromePath, { signal });
