...
export async function launchChrome(chromePath, { signal } = {}) {
  ...
  // Tie the child's life to the abort signal: a cancel/timeout/shutdown during
  // launch or anytime after kills Chrome so no orphan process survives.
  const onAbort = () => {
    try {
      child.kill("SIGKILL");
    } catch {
  ...
  return { child, wsUrl, userDataDir, disposeAbort: () => signal?.removeEventListener("abort", onAbort) };
}
