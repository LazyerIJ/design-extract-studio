...
export function validateJobInput(input) {
  ...
  return {
    ...
    options: {
      ...
      // Opt-in: omitting the flag means no layout extraction (default false).
      layout: booleanOption(options.layout, false, "options.layout"),
    },
  };
}
...
export function validateApplicationId(value) {
  if (typeof value !== "string" || !/^[a-z0-9][a-z0-9-]{7,63}$/i.test(value)) {
    const error = validationError("Invalid application id");
    error.statusCode = 404;
    throw error;
  }
  return value;
...
export function validateAnalyzeInput(input) {
  ...
  if (
    ...
    throw validationError("targetPath is required and must be at most 4096 characters", "targetPath");
  }
  return { targetPath: input.targetPath };
}

export function validateApplicationInput(input) {
  const { targetPath } = validateAnalyzeInput(input);
  if (!["safe", "ai"].includes(input.mode)) {
    throw validationError("mode must be safe or ai", "mode");
  }
  if (input.confirmed !== true) {
    throw validationError("Explicit confirmation is required", "confirmed");
  }
  return { targetPath, mode: input.mode, confirmed: true };
}
