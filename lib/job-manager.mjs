...
export class JobManager extends EventEmitter {
  constructor({ config, store, runner, layoutRunner = runLayoutExtraction }) {
    super();
    this.config = config;
    this.store = store;
    this.runner = runner;
    this.layoutRunner = layoutRunner;
    this.jobs = new Map();
    this.queue = [];
    this.active = new Map();
    this.stopping = false;
  ...
  async create(input, retryOf = null) {
    ...
    const job = {
      ...
      layout: {
        status:
          this.config.enableLayoutExtraction && input.options?.layout === true
            ? "pending"
            : "disabled",
      },
      artifactCount: 0,
  ...
  async execute(job) {
    let cancel = () => {};
    // One controller covers the post-designlang phases (notably layout's Chrome
    // + CDP). Cancelling aborts it AND kills the designlang child via `cancel`.
    const abort = new AbortController();
    this.active.set(job.id, {
      cancel: () => {
        abort.abort(Object.assign(new Error("Job cancelled"), { code: "JOB_CANCELLED" }));
        cancel();
      },
    });
    job.status = "running";
    ...
    try {
      ...
      job.layout = await this.layoutRunner(job, this.config, hooks);
