...
// *same* element at another viewport — independent of which siblings were
// hidden, reordered, or reflowed. The element is matched only if its signature
// is present; we never guess from tree position (the old, mismatch-prone way).
...
function buildCss(data, prefix) {
  ...
  for (const bpName of ["tablet", "mobile"]) {
    ...
    for (const node of containers) {
      const match = node.dom ? byDom.get(node.dom) : null;
      if (!match) {
        // Box not found at this width — don't guess a responsive rule (the old
        // position-based match invented wrong ones).
        skipped[bpName] += 1;
        continue;
      }
